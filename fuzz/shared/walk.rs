...
// is a hard error.
use fs_core::{BlockDevice, BlockRead, Result as BlockResult};

/// An image held in memory, presented as a device.
///
/// `BlockDevice` as well as `BlockRead` because `open_on_device` asks
/// for one; the default `write_at` returns `ReadOnly`, which is the
/// right answer here and keeps the fuzzing read-only.
...
/// How many extents one walk will enumerate.
///
/// A crafted L1 table can describe any number, and enumerating all of
/// them would make a case slow rather than failing it -- which reads as
/// a hang without being one.
pub const EXTENT_BUDGET: usize = 256;

/// How far into the virtual disk a walk will read.
pub const READ_BUDGET: u64 = 1 << 20;

/// Open an image and read what a caller would: the geometry, the extent
/// map, the cluster state at a few offsets, and some data.
///
...
pub fn walk(image: &[u8]) {
    ...
    let Ok(reader) = qcow2::Qcow2Reader::open_on_device(dev) else {
        return;
    };

    let virtual_size = reader.virtual_size();
    let cluster = reader.cluster_size();
    let _ = reader.version();
    let _ = reader.has_backing();
    let _ = reader.is_writable();
    let _ = reader.header().check_supported();

    for extent in reader.extents().take(EXTENT_BUDGET) {
        let _ = extent;
    }

    // Cluster boundaries are where the mapping arithmetic happens, so
    // step by one rather than by an arbitrary stride -- but only while
    // the step is a real number, since a crafted header can claim a
    // cluster size of zero.
    if cluster > 0 {
        let mut at = 0u64;
        let mut steps = 0;
        while at < virtual_size.min(READ_BUDGET) && steps < EXTENT_BUDGET {
            let _ = reader.cluster_status_at(at);
            at = at.saturating_add(cluster);
            steps += 1;
        }
    }
