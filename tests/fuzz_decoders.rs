...
//! `refcount_table_offset` -- and none of that is reachable from a
//! header alone: the mapping is two levels of table, each read from an
//! offset the level above supplied. So the corpus holds five images
//! `qemu-img` wrote, and `image` mutates and opens them.
//!
//! One per shape that changes the read path: the default cluster size,
//! a small one, a version 2 file, a compressed one, and one with a
//! backing reference. A large cluster size is in the header corpus
//! only -- qcow2 gives the refcount table and each mapping level a
//! whole cluster of their own, so a 256 KiB cluster size makes a 1.8 MB
//! file out of two data clusters, which is most of a corpus for one
//! variant.
//!
//! # The corpus is an oracle
//!
//! `qemu-img` wrote a known pattern into each image: `A` at offset 0
//! and `B` at 500,000, with a hole between them.
//! `the_corpus_reads_back_what_qemu_img_wrote` checks that this crate
//! returns exactly that, so the reference implementation is the
//! standard on every pull request -- on a machine with no `qemu-img`
//! installed.
...
fn the_corpus_reads_back_what_qemu_img_wrote() {
    ...
    for (name, bytes) in images {
        ...
        // A backing chain cannot be resolved from a device: the backing
        // file is a NAME, and a name needs a filesystem to look it up
        // in. `open_on_device` says so rather than silently returning
        // the holes as zeros, which would be a read that quietly
        ...
        if name.starts_with("backed") {
            let refused = qcow2::Qcow2Reader::open_on_device(dev)
                ...
                .unwrap_or_else(|| {
                    panic!(
                        "{name}: opening a backed image on a device succeeded, but nothing \
                        on a device can resolve the backing name"
                    )
                });
            let said = refused.to_string();
            assert!(
                said.contains("backing"),
                "{name}: refused, but not for the backing chain: {said}"
            );
            continue;
        }
        ...
        // The hole between them reads as zeros rather than as whatever
        // was last in that cluster, which is the property an
        // unallocated cluster has and the one a mapping error breaks.
        ...
        Err(mpsc::RecvTimeoutError::Timeout) => {
            ...
            let _ = writeln!(
                ...
                "\nhung: no progress for {:?} at {}\n\
                 A decoder did not return. A refcount or mapping table that points back \
