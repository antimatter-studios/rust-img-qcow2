...
//!
//! `include/qcow2.h` said "Link with libam_img_qcow2.a" for as long as
//! the file existed, and cargo has never produced that name: `[lib]
//! name` is `qcow2`, so the artefact is `libqcow2.a` — which is exactly
//! what `chores.yml` copies. A C consumer following the header got a
//! linker error for a library nobody builds.
//!
...
fn the_header_scan_finds_library_names_in_prose() {
    let header = " * Link with libqcow2.a and include this header alongside fs_core.h.\n";
    assert_eq!(libraries_named(header), vec!["libqcow2.a"]);

    // The exact defect, and the near-miss that has to stay distinct.
    assert_eq!(
        libraries_named(" * Link with libam_img_qcow2.a and include this\n"),
        vec!["libam_img_qcow2.a"]
    );
